...
void PeerManager::AllotPeerLevel()
{
    ...
    peers_bandwidth_ = new float[NUM_PEER];

    if(nullptr == peers_bandwidth_) {
	...
	for(int pid = 0; pid < NUM_PEER; pid++)
    {
		...
		peers_bandwidth_[pid] = g_k_peer_level[level-1].trans_time;
